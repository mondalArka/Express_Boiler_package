            ...
            switch (sys) {
                ...
                case "linux": {
                    if (args[5].includes("mongoose")) {
                        ...
                        str["scripts"]["start"] = "npm run build && node ./dist/index.js"
                        str["scripts"]["build"] = "rm -rf dist && tsc -p ."
                        str["scripts"]["generate:migrate"] = "npm run build && typeorm migration:generate src/migrations/${TABLE_NAME} -d dist/config/dataSource.js"
                        str["scripts"]["migrate"] = "npm run build && typeorm migration:run -d dist/config/dataSource.js"
                        str["scripts"]["migrate:rollback"] = "npm run build && typeorm migration:revert -d dist/config/dataSource.js"
                        str["dependencies"] = {
                            "mysql2": "^3.12.0",
                            "typeorm": "^0.3.20",
                            "express": "^4.21.2",
